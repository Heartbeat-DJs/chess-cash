/* ===================================================================
   ChessCash — Friends Service
   Member search, friend requests, and the friends roster.
   =================================================================== */

import { randomUUID } from 'crypto';
import { getDb } from './db';
import { AuthError } from './auth';

export type FriendStatus = 'none' | 'pending_out' | 'pending_in' | 'friends';

export interface MemberSearchResult {
  id: string;
  username: string;
  rating: number;
  gamesPlayed: number;
  friendStatus: FriendStatus;
}

export interface FriendEntry {
  /** The friendship row id (used to respond/remove). */
  id: string;
  userId: string;
  username: string;
  rating: number;
}

interface FriendRow {
  id: string;
  requester_id: string;
  addressee_id: string;
  status: string;
  created_at: number;
}

function pairRow(selfId: string, otherId: string): FriendRow | undefined {
  return getDb()
    .prepare(
      `SELECT * FROM friends
       WHERE (requester_id = ? AND addressee_id = ?) OR (requester_id = ? AND addressee_id = ?)`
    )
    .get(selfId, otherId, otherId, selfId) as FriendRow | undefined;
}

function statusFor(selfId: string, otherId: string): FriendStatus {
  const row = pairRow(selfId, otherId);
  if (!row) return 'none';
  if (row.status === 'accepted') return 'friends';
  return row.requester_id === selfId ? 'pending_out' : 'pending_in';
}

export function searchMembers(selfId: string, query: string): MemberSearchResult[] {
  const q = query.trim();
  if (q.length < 2) return [];
  const rows = getDb()
    .prepare(
      `SELECT id, username, rating, games_played FROM users
       WHERE username LIKE ? AND id != ?
       ORDER BY username COLLATE NOCASE LIMIT 10`
    )
    .all(`%${q.replace(/[%_]/g, '')}%`, selfId) as {
    id: string;
    username: string;
    rating: number;
    games_played: number;
  }[];

  return rows.map((r) => ({
    id: r.id,
    username: r.username,
    rating: r.rating,
    gamesPlayed: r.games_played,
    friendStatus: statusFor(selfId, r.id),
  }));
}

/** Send a friend request; a crossing request auto-accepts. */
export function sendFriendRequest(selfId: string, username: string): FriendStatus {
  const db = getDb();
  const target = db.prepare('SELECT id FROM users WHERE username = ?').get(username) as
    | { id: string }
    | undefined;
  if (!target) throw new AuthError('No member by that name.', 404);
  if (target.id === selfId) throw new AuthError('You are already your own best company.');

  const existing = pairRow(selfId, target.id);
  if (existing) {
    if (existing.status === 'accepted') return 'friends';
    if (existing.requester_id === selfId) return 'pending_out';
    // They asked first — crossing requests means both want it
    db.prepare(`UPDATE friends SET status = 'accepted' WHERE id = ?`).run(existing.id);
    return 'friends';
  }

  db.prepare(
    `INSERT INTO friends (id, requester_id, addressee_id, status, created_at)
     VALUES (?, ?, ?, 'pending', ?)`
  ).run(randomUUID(), selfId, target.id, Date.now());
  return 'pending_out';
}

export function respondFriendRequest(selfId: string, requestId: string, accept: boolean) {
  const db = getDb();
  const row = db.prepare('SELECT * FROM friends WHERE id = ?').get(requestId) as FriendRow | undefined;
  if (!row) throw new AuthError('Request not found.', 404);
  if (row.addressee_id !== selfId) throw new AuthError('Not your request to answer.', 403);
  if (row.status !== 'pending') throw new AuthError('Already settled.');

  if (accept) {
    db.prepare(`UPDATE friends SET status = 'accepted' WHERE id = ?`).run(requestId);
  } else {
    db.prepare('DELETE FROM friends WHERE id = ?').run(requestId);
  }
}

export function removeFriend(selfId: string, friendshipId: string) {
  const db = getDb();
  const row = db.prepare('SELECT * FROM friends WHERE id = ?').get(friendshipId) as FriendRow | undefined;
  if (!row) return;
  if (row.requester_id !== selfId && row.addressee_id !== selfId) {
    throw new AuthError('Not your friendship to end.', 403);
  }
  db.prepare('DELETE FROM friends WHERE id = ?').run(friendshipId);
}

export function listFriends(selfId: string): {
  friends: FriendEntry[];
  incoming: FriendEntry[];
  outgoing: FriendEntry[];
} {
  const db = getDb();
  const rows = db
    .prepare(
      `SELECT f.id, f.requester_id, f.addressee_id, f.status,
              u.id AS other_id, u.username, u.rating
       FROM friends f
       JOIN users u ON u.id = CASE WHEN f.requester_id = ? THEN f.addressee_id ELSE f.requester_id END
       WHERE f.requester_id = ? OR f.addressee_id = ?
       ORDER BY u.username COLLATE NOCASE`
    )
    .all(selfId, selfId, selfId) as {
    id: string;
    requester_id: string;
    addressee_id: string;
    status: string;
    other_id: string;
    username: string;
    rating: number;
  }[];

  const friends: FriendEntry[] = [];
  const incoming: FriendEntry[] = [];
  const outgoing: FriendEntry[] = [];
  for (const r of rows) {
    const entry: FriendEntry = { id: r.id, userId: r.other_id, username: r.username, rating: r.rating };
    if (r.status === 'accepted') friends.push(entry);
    else if (r.addressee_id === selfId) incoming.push(entry);
    else outgoing.push(entry);
  }
  return { friends, incoming, outgoing };
}
